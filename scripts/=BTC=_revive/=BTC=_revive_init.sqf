...
BTC_who_can_revive  = ["B_medic_F","B_recon_JTAC_F","B_recon_medic_F"];
BTC_loop_check      = 1;
...
if (!isDedicated) then {
	...
	BTC_Medical_Trucks = ["O_G_Van_01_transport_F"];
...
if (isServer) then {
	BTC_vehs_mobile_west = [Diver_Respawn,Special_Force_Respawn];		//Editable - define mobile west (this is the vehicle name)
