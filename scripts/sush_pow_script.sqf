...
sush_pow_fn_addActionGetOut = {
	...
	private _actTxt = format ["<t color='#ff8133'>Sors du véhicule (%1)</t>", name _p];
...
sush_pow_fn_followAction = {
		...
		private _actTxt = format ["<t color='#ff8133'>Rentre dans le véhicule (%1)</t>", name _p];
...
private _actionId = _pow addAction ["<t color='#ff8133'>Libérer</t>",sush_pow_fn_takeAction,nil,6,true,true,"","_this distance _target < 3"];
...
while {true} do {
	...
	switch (_pow_state) do {
		case 1: {
			_pow setUserActionText [_actionId, "<t color='#ff8133'>Attends ici</t>"];
		};
		case 2: {
			_pow setUserActionText [_actionId, "<t color='#ff8133'>Suis moi</t>"];
		}
	};
	sleep 1;
};
