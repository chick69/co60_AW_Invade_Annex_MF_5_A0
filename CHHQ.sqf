CHHQ_fnc_deploy = {
	...
	if (count _flatPos isEqualTo 0) exitWith {systemChat format ["Vous ne pouvez déployer le HQ ici, il faut au moins (%1m) mètres de place libre.", round _radius]};
	...
	*/
	...
	[[_veh, ["<t color='#FF0000'>Démonter le HQ", "_this spawn CHHQ_fnc_undeploy", [_side, _cargoInfo, _composition], 0, false, true, "", "[_target, _this] call CHHQ_fnc_actionConditions"]], "CHHQ_fnc_addAction", _side] call BIS_fnc_MP;
...
CHHQ_fnc_undeploy = {
	...
	_veh allowDamage true;
	{_x setPos ((getPosASL _x) findEmptyPosition [0, 25, "CAManBase"]); _x allowDamage true; _x enableSimulationGlobal true} forEach _nearestPlayers;
		[[_veh,["<t color='#FF0000'>Déployer le HQ", "_this spawn CHHQ_fnc_deploy", [_side, _cargoInfo, _composition], 0, false, true, "", "[_target, _this] call CHHQ_fnc_actionConditions"]], "CHHQ_fnc_addAction", _side] call BIS_fnc_MP;
...
CHHQ_fnc_startingSetup = {
	...
	if (_veh getVariable ["CHHQ_deployed",false]) then {
		...
		if (playerSide isEqualTo _side) then {		
			...
			_id = _veh addAction ["<t color='#FF0000'>Démonter le HQ", "_this spawn CHHQ_fnc_undeploy", [_side, _cargoInfo, _composition], 0, false, true, "", "[_target, _this] call CHHQ_fnc_actionConditions"];
			_veh setVariable ["CHHQ_actionID",_id];
		};
	} else {
		_veh lock false;
		...
		if (playerSide isEqualTo _side) then {
			...
			_id = _veh addAction ["<t color='#FF0000'>Déployer le HQ", "_this spawn CHHQ_fnc_deploy", [_side, _cargoInfo, _composition], 0, false, true, "", "[_target, _this] call CHHQ_fnc_actionConditions"];	
			_veh setVariable ["CHHQ_actionID",_id];	
...
switch (toLower typeOf _obj) do {
	...
	case (toLower "O_G_Van_01_transport_F"): {
