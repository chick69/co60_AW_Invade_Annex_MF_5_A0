...
if (isNil "dep_useheadless")        then { dep_useheadless      = true; };          // Load DEP on a headless client
if (isNil "dep_headlessclient")     then { dep_headlessclient   = "HeadlessClient"; };  // Specify the headless client if there are more than one
...
if (isNil "dep_town_occupation")    then { dep_town_occupation  = 35; };            // Percentage of towns that are occupied
...
switch (dep_side) do 
{
    case east: {
        ...
        if (isNil "dep_u_soldier")      	then { dep_u_soldier    	= "O_G_Soldier_F"; };
        if (isNil "dep_u_gl")           	then { dep_u_gl         	= "O_G_Soldier_GL_F"; };
        if (isNil "dep_u_ar")           	then { dep_u_ar         	= "O_G_Soldier_AR_F"; };
        if (isNil "dep_u_at")           	then { dep_u_at         	= "O_G_Soldier_LAT_F"; };
        if (isNil "dep_u_medic")        	then { dep_u_medic      	= "O_G_medic_F"; };
        if (isNil "dep_u_aa")           	then { dep_u_aa         	= "O_G_engineer_F"; };
        if (isNil "dep_u_aaa")          	then { dep_u_aaa        	= "O_G_Soldier_exp_F"; };
        if (isNil "dep_u_sl")           	then { dep_u_sl         	= "O_G_Soldier_SL_F"; };
        if (isNil "dep_u_marksman")     	then { dep_u_marksman   	= "O_G_Soldier_M_F"; };
        if (isNil "dep_u_sniper")       	then { dep_u_sniper     	= "O_G_officer_F"; };
        if (isNil "dep_u_veh_cmnd")     	then { dep_u_veh_cmnd   	= "O_G_officer_F"; };
        if (isNil "dep_u_veh_crew")     	then { dep_u_veh_crew   	= "O_G_Soldier_F"; };
        
        if (isNil "dep_ground_vehicles") 	then { dep_ground_vehicles 	= ["I_C_Offroad_02_unarmed_F","I_C_Van_01_transport_F","I_G_Van_01_transport_F","I_G_Offroad_01_armed_F","O_T_Truck_03_transport_ghex_F"]; };
