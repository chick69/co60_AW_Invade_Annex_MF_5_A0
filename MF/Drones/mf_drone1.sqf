...
if (_typeOf in _reserved_slots) then {
	if (isNil "VDRONE1") then {
		_okOk = 1;
	} else {
		if (!alive VDRONE1) then {_okOk = 1;};
	};

	if (_okOk==1)  then {
		VDRONE1 = createVehicle ["B_UAV_02_CAS_F", getMarkerPos "DRONE1", [], 0,""];  
		createVehicleCrew VDRONE1;
		VDRONE1 setDir 90; 
		{
			_x addCuratorEditableObjects [units VDRONE1, false];
